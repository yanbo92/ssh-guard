...
const SESSION_WIDE_APPROVAL_TEXT = "I understand the risk. Allow all SSH commands for this session.";
...
function isApprovalText(text: string): boolean {
  ...
  if (normalized.includes("not ") || normalized.includes("don't") || normalized.includes("do not")) {
    return false;
  }
  return (
    normalized === "allow" ||
  ...
  api.on("before_tool_call", (event, ctx) => {
    ...
    if (isSshExec && state?.awaitingApproval) {
      return {
        block: true,
        blockReason:
          "Execution is paused pending approval. Briefly explain the command and show it in a plain Markdown code block without a language tag, using exactly ``` followed by the command and then ```. Tell the user they can reply with exactly one of: allow / approve / proceed / yes for one-time approval, or exactly: I understand the risk. Allow all SSH commands for this session. for session-wide approval, which may not be read-only and may break the user's machine." +
    ...
    return {
      block: true,
      blockReason:
        "SSH commands are blocked until approved. Briefly explain the command and show it in a plain Markdown code block without a language tag, using exactly ``` followed by the command and then ```. Tell the user they can reply with exactly one of: allow / approve / proceed / yes for one-time approval, or exactly: I understand the risk. Allow all SSH commands for this session. for session-wide approval, which may not be read-only and may break the user's machine." +
